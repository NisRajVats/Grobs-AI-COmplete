...
from .heuristic_parser import extract_text_from_file
...
class ParsedResumeWithConfidence(BaseModel):
    ...
    def to_structured_resume(self) -> StructuredResume:
        ...
        return StructuredResume(
            ...
            education=[EducationEntry(**e) for e in self.education.value] if self.education.value and isinstance(self.education.value[0], dict) else self.education.value,
            experience=[ExperienceEntry(**e) for e in self.experience.value] if self.experience.value and isinstance(self.experience.value[0], dict) else self.experience.value,
            projects=[ProjectEntry(**p) for p in self.projects.value] if self.projects.value and isinstance(self.projects.value[0], dict) else self.projects.value,
            skills=[SkillEntry(**s) for s in self.skills.value] if self.skills.value and isinstance(self.skills.value[0], dict) else self.skills.value,
...
def parse_resume_ensemble(path_or_text: str, use_llm: bool = True) -> Dict[str, Any]:
    """
    Backward-compatible resume parsing function.
    
    Args:
        path_or_text: Path to resume file or raw resume text
        use_llm: Whether to use LLM parsing
        
    Returns:
        Parsed resume dictionary (same format as original parser)
    """
    # Check if input is a path or raw text
    if os.path.isfile(path_or_text) or len(path_or_text) < 260:
        try:
            text = extract_text_from_file(path_or_text)
...
async def parse_resume_async(path: str, use_llm: bool = True) -> Dict[str, Any]:
    """Async wrapper for backward compatibility."""
    import asyncio
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, parse_resume_ensemble, path, use_llm)


# Alias for backward compatibility
parse_resume = parse_resume_ensemble


def extract_text_from_file(file_path: str) -> str:
    """Wrapper for text extraction."""
    from .heuristic_parser import extract_text_from_file as _extract
...
def extract_email(text: str) -> Optional[str]:
    from .heuristic_parser import extract_email as _extract
    return _extract(text)


def extract_name(text: str) -> str:
    from .heuristic_parser import extract_name as _extract
    return _extract(text)


def extract_skills_from_text(text: str) -> List[Dict]:
    from .heuristic_parser import extract_skills_from_text as _extract
    return _extract(text)


def parse_resume_with_llm(text: str, provider: Optional[str] = None) -> Optional[Dict]:
    from .heuristic_parser import parse_resume_with_llm as _parse
    return _parse(text, provider)


def clean_text(text: str) -> str:
    from .heuristic_parser import clean_text as _clean
    return _clean(text)


def extract_phone(text: str) -> Optional[str]:
    from .heuristic_parser import extract_phone as _extract
    return _extract(text)


def extract_linkedin(text: str) -> Optional[str]:
    from .heuristic_parser import extract_linkedin as _extract
    return _extract(text)


def extract_github(text: str) -> Optional[str]:
    from .heuristic_parser import extract_github as _extract
    return _extract(text)


def extract_portfolio(text: str) -> Optional[str]:
    from .heuristic_parser import extract_portfolio as _extract
    return _extract(text)


def extract_title(text: str) -> Optional[str]:
    from .heuristic_parser import extract_title as _extract
    return _extract(text)


def extract_sections(text: str) -> Dict[str, List[str]]:
    from .heuristic_parser import extract_sections as _extract
    return _extract(text)


def extract_education_details(lines: List[str]) -> List[Dict]:
    from .heuristic_parser import extract_education_details as _extract
    return _extract(lines)


def extract_experience_details(lines: List[str]) -> List[Dict]:
    from .heuristic_parser import extract_experience_details as _extract
    return _extract(lines)


def extract_project_details(lines: List[str]) -> List[Dict]:
    from .heuristic_parser import extract_project_details as _extract
    return _extract(lines)


def extract_skills(lines: List[str]) -> List[Dict]:
    from .heuristic_parser import extract_skills as _extract
    return _extract(lines)
